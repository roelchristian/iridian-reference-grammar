...
\subsection{Complement clauses in general}\label{sec:complement-clauses}

There are two main types of complement clauses in Iridian: quotative clauses
with \ird{by} and subordinate clauses with \ird{-no}. Both require its verb
...
\a {[Tereza Jankám hloubževě by]\(\lsub{CP}\) Markám bych zěknik.}
\a {Markám [Tereza Jankám hloubževě by]\(\lsub{CP}\) bych zěknik.}
\a {Markám bych [Tereza Jankám hloubževě by]\(\lsub{CP}\) zěknik.}
\xe

\subsection{Quotative constructions}\label{sec:reported-speech}
...
\ird{by}. Direct quotations do not require the conjunctive form, although they are
still separated from the main clause by \ird{by}.

\pex
  \begingl
    \gla Koleč sní polšice by Lukáš zíček.//
    \glb key \Refl{}.\Acc{} lose-\Av{}-\Pf{}-\Cnj{} \Quot{} Lukáš say-\Av{}-\Pf{}//
    \glft \trsl{Lukáš said he lost his keys.}//
  \endgl
\xe
...
main exception being the use of the reflexive \ird{se} if the subject of the
...
clause are called {\sc bare quotatives}\index{bare quotative}.

\pex
\begingl
  \gla Krasouv Markám nakdenice By.//
  \glb jewelry Marek-\Agt{} steal-\Pv{}-\Pf{}-\Cnj{} \Quot{}//
  \glft \trsl{(I heard that) Marek stole the jewelry.}//
\endgl
\xe
...
clause is terminated by the conjunctive ending \ird{-no} instead of \ird{by}.
...
proposition. This usage is similar to the use of a conjunctive ending \ird{-má}
...
\pex
  \a
  \begingl
  \glpreamble \lingcontext{The speaker ironically comments about another customer in the shop who is complaining at the counter, blind with rage.}//
  \gla Děne všihnevě by.//
  \glb \Spec{} angry-\Cont{}-\Cnj{} \Quot{}//
  \glft \trsl{You can tell he's angry.}//
  \endgl
\xe

When the bare quotative is used to report speech and not to express the
speaker's judgment, the reportative particle \ird{kamo} is used to make it clear
that the statement is in fact reported speech. Although it does not pass a
judgment on the truth value of the statement, \ird{kamo} makes it clear that the
statement did not come from the speaker and that the use of the quotative is not
a hedging strategy. \ird{Kamo} is particularly common in newscasts or in other
...
\ird{Kamo} is often dropped when the main clause is present, although it might
...
\begingl
  ...
  \glft \trsl{He said that they will have dinner at the \emph{Hospoda} tonight.}//
\endgl
\xe
...
\subsection{Complementation with \ird{no}}
