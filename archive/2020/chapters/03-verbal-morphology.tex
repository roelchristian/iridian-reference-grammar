...
\begin{figure}
	...
	\begin{tblr}{
		...
		aspect\index{aspect} &
		mood\index{mood} &
		conjunctive\index{conjunctive form} \\ 
		\bottomrule 
	...
	\begin{tblr}{width=\textwidth,colspec={X[1.2]XXXX[1.4]XXX[1.2]}, rowsep=1pt}
		...
		& & & & & & & \\  % empty line
		\SetCell[c=8]{} \irdp{zápiaštébice by}{(He) was said to not have fed.}\\ 
		...
		& & & & & & & \\  % empty line
		\SetCell[c=8]{} \irdp{zánepiaštébalnile by}{If he were said to not have been able to make someone feed.}\\ 
...
followed by the conjunctive suffix \ird{-e}, with the final \ird{-k} in the latter
...
form is also called the `conjunctive mood' in other grammars (in contrast to the
conjunctive form discussed in \S\,\ref{sec:conjunctive-form}), after the name in
...
verb must be further marked in the c to indicate that the speaker is not
...
\section{Conjunctive form}\label{sec:conjunctive-form}
\index{conjunctive form}

The {\sc conjunctive form} (called \irdp{běževí muk}{linking form or bridging
form} in Iridian) is used in verbs appearing in secondary clauses in a sentence.
It originated as a way to mark reported speech and evidentiality in Old Iridian,
and while the conjunctive form is still used for this purpose in Modern Iridian,
the usage has expanded to include relative clauses, subordination and other
forms of clause linking. The use of the conjunctive form is discussed in detail
in \S\,\ref{sec:clause-linking}. The conjunctive form of a verb is formed by adding the suffix \ird{-e} to the fully-conjugated verb, often resulting to the palatalization of the final consonant in the suffix. Nonetheless, the resulting may appear largely irregular; they are listed in Table \ref{tbl:conjunctive}.
\begin{table}
\footnotesize \sffamily
	\caption{Sound changes when deriving the conjunctive form.}
	\medskip
	\label{tbl:conjunctive}
