% path: preamble/commands.tex
% CUSTOM COMMANDS


\newcommand{\sx}[1]{\textsuperscript{#1}}       % superscript phoneme
\newcommand{\mk}[1]{\textsc{#1}}				% glossing abbreviation      
\newcommand{\nf}[1]{\normalfont{#1}}             % normal font
\newcommand{\tss}[1]{\textsubscript{#1}}
\newcommand{\tsup}[1]{\textsuperscript{#1}}
\newcommand{\ird}[1]{\textit{#1}}				% text in Iridian
\newcommand{\foreign}[1]{\textit{#1}}           % text in another language
\newcommand{\rec}[1]{*\textit{#1}}				% reconstruction in Old Iridian
\newcommand{\trsl}[1]{`#1'}					    % translation in quotes
\newcommand{\irdp}[2]{\ird{#1}, \trsl{#2}}      % Iridian text with translation
\newcommand{\irdU}[1]{#1}                       % non-italicised Iridian text
\newcommand{\ttla}[1]{`#1'}						% title of an article, short story setcounter
\newcommand{\ttlb}[1]{\emph{#1}}				% title of a book
\newcommand{\orth}[1]{$\langle$#1$\rangle$}     % text in <brackets>
\newcommand{\phon}[3]{\ird{#1} [#2] \trsl{#3}}  % Iridian text with phonetic transcription and translation
\newcommand{\su}[1]{$_{#1}$}                    % subscript 
\newcommand{\SubjI}{\Subj{}.\Ipf{}}
\newcommand{\SubjP}{\Subj{}.\Pf{}}
\newcommand{\asp}[1]{{#1}\textsuperscript{h}}   % aspirated TODO: delete instances of this
\newcommand{\lex}[2]{\textit{#1} #2}        % lexicon entry
\newcommand{\lingcontext}[1]{{\small {\emph{Context:} #1}}} % additional context about an example sentence
\newcommand{\lsub}[1]{_{\textrm{#1}}}           % subscripts in linguistics-related text