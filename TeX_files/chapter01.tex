...
\begin{table}[h!]
	...
	\begin{longtabu}to \textwidth {>{\bfseries}M>{\itshape}M[2]M>{\bfseries}M>{\itshape}M[2]M}
		...
		A a & á & \bt{6} & N n & en & \bt{n}\\
		
		Á á & necs ko á & \bt{O:}& O o & ó & \bt{O}\\
	
		Ai ai & ái & \bt{\ae}&	Ó ó & necs ko ó & \bt{o:}\\
		
		Ái ái & necs ko ái & \bt{\ae:}& Oi oi & oi & \bt{\o}\\
		
		B b& bé & \bt{b}& Ói ói & necs ko ói & \bt{\o:}\\
		
		C c& cet & \bt{\ttb{ts}}&P p & pé & \bt{p}\\
		
		Cs cs& cset & \bt{\ttb{tS}}&Q q & kvé & --\\
		
		D d&  dé & \bt{d}&R r & er & \bt{R}\\
		
		Dc dc&  ódcet & \bt{\ttb{dZ}}&S s & es & \bt{s}\\
		
		Dz dz&  ódzet & \bt{\ttb{dz}}& Sz sz & esz & \bt{S}\\
		
		E e&  é & \bt{E}& T t & té & \bt{t}\\
		
		É é&  necs ko é & \bt{e:}&	U u & ú & \bt{U} \\
		
		F f&  fý & --& Ú ú & necs ko ú & \bt{u:}\\
		G g&  gé & \bt{g}&Ui ui & ui & \bt{Y}\\
		H h&  há & \bt{x}&Úi úi & necs ko úi & \bt{y:}\\
		I i&  í & \bt{I}&V v & vé & \bt{v}\\
		Í í& necs ko í & \bt{i:}&W w & vénék & --\\
		J j&  jóit & \bt{j}&	X x & iks & --\\
		K k & ká & \bt{k}&Y y & ýpsýlon & \bt{I}\\
		L l & el & \bt{l}&Ý ý & necs ko ýpsýlon & \bt{i:}\\
		M m & em & \bt{m}&Z z & zet & \bt{z} \\
		&&&Zs zs & zses & \bt{Z}\\
		\bottomrule
