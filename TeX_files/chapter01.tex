...
\ex{\small
	\begin{tabularx}{0.8\textwidth}{>{\bfseries}YYY}
		niho		& \bt{\nn{}IxO} & `nothing'\\
		tramvek		&\bt{"tr\~5\~wvEx}&`tramway'\\
		sinv\'oní		&\bt{C\~I"vo:\nn i:}&`symphony'\\
		banki\v{s}t	& \bt{"b\~5\~wcICt} & `banker'\\
	\end{tabularx}
}
\xe
...
\begin{enumerate}
	...
	\end{enumerate}
		...
		\item \bt{\ctz} before \bt{b} or \bt{d}:
\begin{enumerate}
	\item \bt{zb}
	\item \bt{zd}:
\end{enumerate}

		\item \bt{S} or \bt{\ctz} followed by a nasal, a liquid, or \bt{v}:
	\begin{enumerate}
		\item \bt{Sm}:
		\item \bt{Sn}:
		\item \bt{Sr}:
...
\end{enumerate}
	...
	\item \bt{\ctz}-voiced stop-\bt{r} clusters
	\begin{enumerate}
		\item \bt{zbr}:
		\item \bt{zdr}:
		\item \bt{zgr}:
...
\begin{table}
	...
	\begin{tabu}to \textwidth {>{\bfseries}MM[2]M>{\bfseries}MM[2]M}
		...
		L l 		& el 	& \bt{l}		& Z z		& zet 		& \bt{\ctz}\\\addlinespace
