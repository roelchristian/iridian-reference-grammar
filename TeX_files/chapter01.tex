...
Iridian has six pairs of corresponding long and short vowels. With the exception of \bt{5~a:} and \bt{u~u:}, long vowels are tenser than their short counterparts. In addition standard Iridian also features the high central vowel \nt{1} as an allophone of \nt{E} and \nt{I} in unstressed positions.
...
Note that since \ird{y} is used to represent palatalization of a consonant in coda position, word-final short \bt{y} sound is written as \ird{\"y}. The rounded vowels \bt{Y~y:} and \bt{\oe~\o:} are diphthongized to \nt{Y\tsa{5}~y:\tsa{5}} at the end of a word.
