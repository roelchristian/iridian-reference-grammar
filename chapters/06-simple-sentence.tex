...
architecture. This section sets out the main assumptions and terminological
conventions used throughout both chapters and relates them to the particle
analysis developed in Chapter~\ref{ch:function-words}.

\subsection{Clause structure and the functional sequence}
\label{sec:th-clause-structure}

The Iridian clause is built up from the predicate outward through a structured
sequence of functional projections. Table~\ref{tab:th-clause-spine} gives the
full spine from innermost to outermost; the projections directly implicated in
the particle system are discussed in detail in
...
One projection in the spine requires special comment. \synhead{NegP} (projection
4) has a single structural position in all Iridian clause types, but its
...
Neg\textsuperscript{0} is spelled out as the free morpheme \ird{staly}, which
surfaces in the canonical clause-final position expected of a head-final
\synhead{NegP} head. In existential clauses, Neg\textsuperscript{0} triggers
suppletive root replacement: the existential root \ird{jec-} is replaced by
\ird{zad-}, and Neg\textsuperscript{0} itself has no independent PF exponent.
These are three phonological reflexes of one syntactic head, not three
independent negation strategies; the structural position of \synhead{NegP} is
the same in all three cases.

Within the zero-copula domain, a further distinction in semantic scope arises
from the type of the predicative complement. When the predicate is a plain
\synhead{DP}, \ird{staly} produces predicational negation. When the predicate is
a participant nominal (a nominalised verb form carrying its own internal
argument structure; see §\,\ref{sec:nmlz-predicative} of
Chapter~\ref{ch:verbs}), the zero-copula equative becomes identificational and
\ird{staly} produces identificational negation --- the denial that the topic
\synhead{DP} is the unique satisfier of the nominalised description, with the
contrastive implication that the correct referent exists. This scope difference
does not require any modification to the clause spine or to the analysis of
\synhead{NegP}; it is a consequence of the semantics of the predication that
\synhead{NegP} scopes over. The full analysis is developed in
§\,\ref{sec:vm-neg-exponents} and §\,\ref{sec:vm-ident-neg} of the verbal
morphosyntax companion and §\,\ref{sec:clausal-negation} of
...
clause spine. \section{Questions: descriptive recap}\label{sec:questions}

The syntactic analysis of Iridian interrogatives is developed in full in
§\,\ref{sec:th-left-periphery}--§\,\ref{sec:th-wh-interrogation}. The present
section provides a brief descriptive summary for cross-reference and points
forward to the relevant complement-clause and discourse chapters.

\subsection{Polar questions}

Polar (yes-no) questions are formed by the overt particle \ird{by} in
Pol\textsuperscript{0} (§\,\ref{sec:th-polar-questions}). In colloquial speech a
rising intonation pattern may suffice without overt \ird{by}; this is a prosodic
reduction of the same clause type, not a distinct syntactic construction. The
Class~1 phasal particles \ird{v} and \ird{s} may co-occur with \ird{by} to
specify the phasal profile of the question (§\,\ref{sec:class1-particles}).
...
Pol\textsuperscript{0} head, which may be phonologically null or spelled out as
overt \ird{by}; both options are standardly available. The wh-operator may
...
not in interrogative well-formedness. A content question with overt \ird{by} is
more explicitly typed as a direct question, while one with null
Pol\textsuperscript{0} is the lighter, more neutral form; the choice is a matter
of discourse profile, not grammatical markedness. The full structural analysis
and four-combination paradigm are in §\,\ref{sec:th-wh-interrogation}.
