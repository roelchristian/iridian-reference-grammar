...
\begin{table}[ht]
	...
	\begin{tabularx}{\textwidth}{l L L l}
		...
		Purposive converb + topic-coreferent pivot &
...
absolutive pivot of the following clause. Because converb clauses themselves are
nominative-aligned, this default often looks superficially like a same-subject
(SS) pattern, but the broader generalisation is one of \term{same-pivot} or
\term{same-topic} continuity rather than an autonomous subject relation. When
...
itself; Iridian has no grammaticalised switch-reference clitic. Where both
clauses supply full nominal pivots, disambiguation is immediate; where one
pivot is omitted, recoverability from discourse context determines the SS/DS
reading.
...
The simultaneous converb (\ird{-ěc}) describes an action that is ongoing or co-occurring with the main clause event, and also functions as a manner converb. The suffix belongs to the palatalizing domain of front-vocalic morphology (see Section~\ref{sec:front-palatalisation}), not to the special sibilant-replacement system of the antipassive. When the converb and main clause have different subjects, the sequential \ird{-ení} is preferred for purely simultaneous meaning.
...
expressed by two converb suffixes, summarised in
...
\begin{table}[ht]
	...
	\begin{tabularx}{\textwidth}{L l L L}
		...
		\ird{-ic}   & if             & open, neutral, contingent, generic &
			does not trigger palatalisation \\[4pt]
...
\ird{-ir}) are lexically exceptional items at \synhead{MoodP}, not productive
...
converb \ird{-ic} does not trigger palatalisation of the thematic
consonant. This is a historical accident: the suffix was grammaticalised
...
palatalizing domain (see §\,\ref{sec:front-palatalisation}).
...
palatalisation and does not interact with the sibilant-replacement system
...
the \ird{-ic} or \ird{-ezmy} clause is by default coreferent with the
absolutive pivot of the matrix clause. Different-pivot (DS) readings require
overt nominal pivots or demonstratives in one or both clauses, or must be
supported by discourse context; Iridian has no grammaticalised switch-reference
clitic (see §\,\ref{sec:switch-reference}). In many everyday examples this will
coincide with same-agent reference, but that is a consequence of the converb
clause's nominative alignment, not evidence for a language-wide subject pivot.
...
the \synhead{MoodP} layer that licenses directive, hortative, and irrealis readings.
...
predicate-internal prefix \ird{zá-} at a separate Neg\textsuperscript{0} node, not by
...
[\textsc{+neg}] is a lexically inherent feature of \synhead{MoodP} exponents
...
clauses} are not analysed here as ordinary adjuncts lacking their host
...
an adjunct to the matrix \synhead{TP}/\synhead{IP}, scoping over the matrix proposition
...
In the insubordinate use, no matrix \synhead{TP} is projected. Instead, the conditional
converb phrase is selected by a null framing head (Frame\textsuperscript{0}) and
the resulting FrameP is integrated into the speech-act projection
(Figure~\ref{fig:cond-insub}).
...
The null Frame\textsuperscript{0} head licenses the conditional converb phrase as an
...
inference. In DM terms, Frame\textsuperscript{0} is a null exponent subject to
...
at most a \synhead{vP}/\synhead{VoiceP}-level adjunct clause, not a full finite clause.
...
as the complement of modal, desiderative, and motion predicates. Palatalization
is irregular, applying mainly to Class~I stems. As the canonical non-finite
...
\subsection{The purposive-motion construction}\label{sec:purp-motion}
...
\subsubsection{Literal purposive motion}\label{sec:purp-motion-literal}
...
In each example the subject is null, recovered from the preceding discourse
context under the general topic-drop pattern. Case marking inside the purposive
...
case---hence \ird{Josifi} (Josif.\Ind{}) in (\ref{ex:purp-meet}), and
...
absolutive (direct) case when overt. The perfective suffix \ird{-nik} is
...
The stem of \ird{razeň-} `smoke' undergoes weak-\textit{e} deletion before the
purposive suffix, yielding \ird{raznit} (§\,\ref{sec:purp-motion-structure}).

\subsubsection{Prospective motion and purposive
...
\begingl
\gla By že pomadit pritnik?//
\glb \textsc{q} \textsc{1sg}.\Ind{} talk.to-\Purp{} come-\Pf{}//
\glft \trsl{Did you come here to talk to me?}//
\endgl
\xe

In (\ref{ex:purp-come}), \ird{prit-} `come' encodes deictic arrival toward the
speaker. The polar question particle \ird{by} occupies the left-peripheral PolP
position, as in all polar interrogatives. The purposive complement \ird{že
pomadit} `to talk to me' precedes the matrix verb in the expected head-final
order; \ird{že} is the indirect-case (oblique) form of the first-person singular
...
\subsubsection{Colloquial extension: intention and
...
approximates the meaning of intention or near-future imminence. This is
precisely the discourse environment that gave rise to the matrixless intentional
future described in §\,\ref{sec:purp-intentional}: once the motion component
weakens sufficiently, the purposive converb alone comes to carry the
prospective-intentional force and the matrix motion verb becomes droppable. This
colloquial extension is register-marked; it represents a diachronic
grammaticalization trajectory rather than a synchronic grammatical pattern and
should be distinguished from the core purposive-motion construction.

\subsubsection{Structural analysis}\label{sec:purp-motion-structure}
...
For weak-\textit{e} stems such as \ird{razeň-} `smoke' (oblique stem
\ird{razn-}), the purposive suffix \ird{-it} is appended to the stem after
deletion of the weak vowel, yielding \ird{raznit}. The clause-final perfective
suffix \ird{-nik} is inserted at
...
\subsubsection{Limits of the construction: contrast with aspectual
...
in colloquial speech is a matter of usage and ongoing grammaticalization
...
\begingl
\gla Mé autobusam zrolit nočknik.//
...
spine under \synhead{AspP}. A form such as \ird{byl nesu} would therefore mean something
...
and \ird{poby} are therefore best analysed as heads of \synhead{CP} selecting a clause
that may itself project \synhead{PolP}; the complementizer contrast is a
...
% grammaticalised converb forms — to be documented.
% ============================================================
...
summarised in Table~\ref{tab:th-clause-spine} and
...
of wh-operators --- whether in situ or fronted to Spec,\synhead{FocP} --- as
...
clause-level interrogative operator in \synhead{PolP} is interpreted as an interrogative
...
spine: \ird{by} occupies \synhead{PolP}, which is above the comment-domain
...
\item[\upshape Polar questions (§\,\ref{sec:th-polar-questions}):] see
	§\,\ref{sec:by-pol-head} (Chapter~\ref{ch:function-words}) for the
