...
clauses are negated, however, Iridian inserts the dedicated negative copula
\ird{staly} rather than using the verbal negative prefix \ird{zá-}, which
requires an overt verbal host.
...
The negative copula is restricted to the negation of equative and
...
At the clausal level, negation is distributed across three constructions. Verbal predicates take the prefix \ird{zá-} (reduced to \ird{za-} in unstressed environments). Negated equative and class-membership clauses use the dedicated negative copula \ird{staly} (§\,\ref{sec:zero-copula}). Existential constructions are negated by the lexically negative verb \ird{zad-} (§\,\ref{sec:zad}), which does not combine with \ird{zá-}. The prohibitive mood suffix \ird{-éma} handles negation of commands. Interaction with the perfective aspect (negated perfective = `should have done but didn't') is discussed in Chapter~\ref{ch:verbs}.
