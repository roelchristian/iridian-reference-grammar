...
\begin{enumerate}[nosep]
	\item {\cscaps aspect}.\index{aspect} Iridian has three primary aspects: perfective, imperfective and contemplative; and two secondary ones: retrospective and prospective.
	\item {\cscaps voice}.\index{voice} Iridian has a strong tendency to leave the topic of the sentence unmarked, instead encoding the primary information on the verb. Due to this, voice must be explicitly marked on the verb. Iridian has the following grammatical voices: agentive, patientive, benefactive, instrumental, locative and reflexive.
	\item {\cscaps mood and modality}.\index{mood} Besides the unmarked indicative, Iridian has the following grammatical moods: subjunctive, conditional, hortative, optative, abilitative, permissive and non-volitive. In addition, secondary prefixes are used to express what would otherwise could be considered as moods: inceptive, causative and reciprocative.
	\item {\cscaps evidentiality}.\index{evidentiality} Iridian formally distinguishes between reported and non-reported information, although actual usage of the former often encompasses various semantic and not strictly evidentiality-related applications.
\end{enumerate}
...
\par The {\cscaps citation form} (or {\cscaps dictionary form}\index{dictionary form|see{citation form}} or {\cscaps lemma}\index{lemma|see{citation form}}) of a verb is the uninflected {\cscaps infinitive}\index{infinitive}, a fossilised form rarely used outside of a very few periphrastic\index{periphrasis} and historical constructions (see \S\,\ref{sec:infinitive}). The infinitive ends with the vowel \ird{-á}, and removing this ending will produce the {\cscaps verb stem}\index{verb stem}. The final consonant  of the stem is called the thematic consonant\index{thematic consonant} and determines the conjugation paradigm the verb follows. The verb stem is a bound form and must always appear with at least one inflectional suffix. 
...
{\cscaps valency} (or {\cscaps valence})\index{valency} is the number of overt arguments a verb\index{argument of a verb} can take in a sentence. \textcite[239]{tesniere1965}, in one of the earliest description of the concept, likens valency by comparing it to bonds between atoms:
\begin{quotation}
	\small
...
Sociative verbs, on the other hand, formed with the prefix \ird{se-} (and glossed here as \Soc{}), indicate actions that are performed with another person or other people. Sociative verbs are formed with the locative voice, with the noun phrase in the topic representing the person/people with whom an action is done. Note that sociative verb constructions cannot be used with inanimate/non-human noun phrases.

\pex
\begingl
\gla Marek bych sezdravounek.//
\glb Marek yesterday \Soc{}-sleep-\Lv{}-\Pf{}//
...
As in most languages, modal constructions in Iridian exhibit significant {\cscaps polysemy}\index{polysemy} (i.e. a single construction can have one or more interpretation depending on the context). For example consider the following sentence:
...
The {\cscaps supine}\index{supine} is a non-finite verb form formed used to indicate necessity or purpose. Both usage has a nominal and a non-nominal form (used similar to an adverb or an adjective), giving the supine a total of four forms, as shown below:
...
\begin{center}
	...
	\begin{longtabu}to \textwidth{Y[0.5]Y}
		...
		\label{verbalder}                             \\
		\toprule
		\multicolumn{1}{c}{\cscaps affix} & \multicolumn{1}{c}{\cscaps examples}                      \\
		\midrule
		\endfirsthead
		%---------------------------------------------------------------%
		...
		\textbf{ce-}\footnote{Verbs in \textbf{ce-} cannot be in the reflexive focus.} + {\cscaps adj}\newline `to cause oneself to become {\cscaps adj}' &
		...
		\textbf{hó-} + {\cscaps noun}\newline `to use {\cscaps n} in a particular way' &
		...
		\textbf{deš-} + {\cscaps noun}\newline `to act in the manner of {\cscaps n}  &
		...
		\textbf{má-iv} + {\cscaps noun}\newline `to so something usually done in {\cscaps noun}'  &
		...
		\textbf{sen-/sem-} + {\cscaps verb}\newline `to {\cscaps verb} incorrectly'  &
