...
\begin{figure}
	...
	\begin{tblr}{width=\textwidth,colspec={X[1.2]XXXX[1.4]XXX[1.2]}}
		...
		aspect\index{aspect} &
		mood\index{mood} &
		quotative\index{quotative} \\ \addlinespace
		\bottomrule \addlinespace

		& & & & & & & \\ \addlinespace % empty line
		\SetCell[c=8]{} \irdp{zápiašček}{(I) did not eat.}\\ \addlinespace
		zá- &
		$\emptyset$ &
		-piašt- &
		-aš- &
		...
		& & & & & & & \\ \addlinespace % empty line
		\SetCell[c=8]{} \irdp{zánepiaštébice}{(He) was said to not have fed.}\\ \addlinespace
		...
		& & & & & & & \\ \addlinespace % empty line
		\SetCell[c=8]{} \irdp{zánepiaštébalnile}{If he were said to not have been able to feed.}\\ \addlinespace
...
A verb in the conditional mood cannot be used as a main verb in a sentence. In
this section we will group these uses into two main categories: conditional
sentences and subordination.

A condition sentence is a statement of the form \trsl{If X, then Y.} Here X is
...
\irdp{ot}{if} or its negative counterpart \irdp{ocy}{if not}, or by
\irdp{bymy}{if} or its negative counterpart \irdp{byž}{if not}. \ird{Bymy} and
\ird{byž} presuppose that the event described in the protasis \emph{will}
happen, but the exact timing of which is yet uncertain. \ird{Ot} and \ird{ocy}
...
of examples above, and counterfactual conditionals, like \trsl{If I had known, I
would have gone.} The latter is used to express a hypothetical situation, i.e.,
...
counterfactual conditionals, only \ird{ot} and \ird{ocy} can be used to
introduce the protasis. To further emphasize the hypothetical nature of the
conditional, the particle \ird{mlada} (glossed as \Hyp{}) may be used before the
...
the regular quotative marker \ird{to-že}.
...
\irdp{teby}{whether} rather than the regular quotative marker \ird{to-že} or
\ird{by}.
...
\footnotesize \sffamily
	...
	\begin{tblr}{width=\textwidth,colspec={X[0.9]X[0.7]X}}
		...
		\midrule \addlinespace
			...
			\multicolumn{3}{@{}l}{{\sc indicative}}\\ \addlinespace
			...
			\multicolumn{3}{@{}l}{{\sc conditional}}\\ \addlinespace
			...
			\multicolumn{3}{@{}l}{{\sc imperative and hortative}}\\ \addlinespace
			...
			\multicolumn{3}{@{}l}{{\sc other forms}}\\ \addlinespace
...
\begin{table}
	...
	\begin{tblr}{width=0.5\textwidth,colspec={X[1.4]X}}
			...
			\midrule \addlinespace
				Potential\index{potential modality} & -aln- \\ \addlinespace
				Desiderative\index{desiderative modality} & -évn- \\ \addlinespace
				Debitive\index{debitive modality} & -ušh- \\ \addlinespace
			\bottomrule
    \end{tblr}
\end{table}
...
aspect. An aspectless participant nominal would always be indefinite in meaning.
