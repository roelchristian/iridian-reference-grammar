...
Sensory verbs are interpreted as non-volitive by default and do not need to be marked with \ird{od-}. Instead, it is volition that must be marked explicitly with the prefix \ird{pě-}. Before verbs with a vowel onset, \ird{} is realized as /pʲě/. \ird{P} however is not fully productive and cannot be used with non-sensory verbs; some sensory verbs may allow neither \ird{od-} nor \ird{pě-}.
...
\begin{table}
    ...
    \begin{tabu}to 0.5\textwidth{@{}Y[1.4]Y@{}}
			...
			\midrule \addlinespace
				...
				Desiderative\index{desiderative modality} & -ušh- \\ \addlinespace
				Debitive\index{debitive modality} & -évn- \\ \addlinespace
			\bottomrule
    \end{tabu}
\end{table}
...
\glb this handwriting too be:small-\Cont{} with read-\mk{pv-npot-sbj.ipf}//
...
\pex \a Nominalized forms of \irdp{piaštou}{to eat} showing a productive morphological {\sc rn}:\smallskip\\
		\vtop{\halign{%
			#\hfil& \qquad #\hfil\cr
...
\a Nominalized forms of \irdp{vadá}{to think} showing a defective morphological {\sc rn} and the alternative lexical {\sc rn}:\smallskip\\
	\vtop{\halign{%
		#\hfil& \qquad #\hfil\cr
