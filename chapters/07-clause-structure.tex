...
\pex
\a Relative clause:\\
  {The man [\textbf{whom} we saw at the store yesterday] is my brother.}
\a Complementation:\\
  {He asked me [\textbf{whether} I had seen the movie.]}
\a Conjunction or clause-linking:\\
  {[\textbf{Although} we went to Dobroslau, we did not see him.]}
\xe
...
ungrammatical. We will call this remnant part the {\sc main clause} (abbreviated
as MC) and the sentence constituent enclosed in square brackets the {\sc
secondary clause} (abbreviated as SC). In general, the verb in the secondary
clause will be marked in the conjunctive form (see \ref{sec:conjunctive-form} in
Iridian. For example, the above sentences would be translated as follows:
...
\a
\begingl
  \gla Do Dobroslava stožicemu ša závednik.//
...
the secondary clause and do not appear as separate morphemes, as in English.
...
clauses; the SC, which occurs first, indicates the first occuring event while
the MC indicates the second event. A sentence can contain multiple SCs, with the
order of the clauses corresponding to the order of the events.
...
\pex
\begingl
  \gla Marku houčicení tětar zaby stojounicení zaby prižek.//
  \glb Mark-\Ins{} meet-\Av{}-\Pf{}-\Cnj{}-and.then theater together
...
If the SC(s) and the MC share the same subject, the chain is usually interpreted
as describing a succession of events, as is the case in the above examples.
However, if the SC(s) and the MC have different subjects, the chain may also
take a `causal' interpretation, i.e., the action in the SC causes the action in
the MC. To be clear, however, using \ird{-ní} is not a direct causation of
causality and this causal reading is a polysemic interpretation of \ird{-ní}.
...
The ending \irdp{-ním}{and} has a similar meaning but offers a vaguer
...
\a\label{ex:nim-perf}
\begingl
  \gla Marek do Praha stožicením pislo Jankám prějenik.//
...
\a\label{ex:nim-prog}
\begingl
  \gla Marek znousčeněním Janek uzdravžime.//
  \glb Marek study-\Av{}-\Prog{}-\Cnj{}-and Janek sleep-\Av{}-\Prog{}//
  \glft \trsl{Marek is studying and Janek is sleeping.}//
\endgl
\xe

Example (\ref{ex:nim-perf}) can be interpreted as either (1) \trsl{Marek went to
...
conversation. In example (\ref{ex:nim-prog}), on the other hand, while the
clauses are still linked by \ird{-ním}, most speakers would likely interpret
...
\section{Conditional clauses}\label{sec:conditional-clauses}
...
mere possibility, but a certainty. This is why \ird{bymy} is used instead of
\ird{ot}.
