...
Verbs (\ird{sl\'adek}) in Iridian are heavily marked. There is a tendency to encode most of the information contained in the sentence on the verb leaving the noun or noun phrase unmarked if possible.
...
\par The citation form (or dictionary form) of a verb is the uninflected infinitive\index{infinitive}, a fossilized form rarely used outside of a very few periphrastic construction. The infinitive ends with the vowel \ird{-\'a}, and removing this ending will produce the verb stem\index{verb stem}. The final consonant  of the stem is called the thematic consonant and determines the conjugation paradigm the verb follows.
...
Where both elements of the agent-patient pair are present in the sentence, one of them is treated as the agent and left unmarked while the other is marked in the comitative\index{comitative} (i.e., \ird{\v{s}e} + instrumental). However, since the action itself is reciprocal, which gets marked as the agent is purely a pragmatic choice. Where one of the members of the agent-patient pair is a pronoun, preference is given to marking the pronoun as the agent (in which case \ird{\v{s}e} is normally ommitted, but with the patient remaining in the instrumental case).
...
\glb \mk{2s.str} Mi\v{s}ek-\mk{inst} \mk{rec}-know-\mk{av-ret} \mk{q}//
\glft \trsl{You and Mi\v{s}ek already met each other right?}//
\endgl
\xe
...
\begin{table}
    \footnotesize\sffamily
		\caption{Conjugation of the verb \ird{pia\v{s}t\'a} in the hortative mood.}
		\label{tbl:hortative}
    \begin{tabu}to 0.7\textwidth{YYY}
         \toprule
         ...
         Instrumental &
         \ird{dopia\v{s}t\'omk\'a} &
         \ird{dopia\v{s}t\'omku}\\

         Reflexive &
         \ird{upia\v{s}\v{c}k\'a} &
         \ird{upia\v{s}\v{c}ku}\\
...
To further soften command, the expression \ird{am luhnink\'a} (from the hortative
form of the verb \irdp{luhn\'a}{to give thanks}, now obsolete except for this
specific usage) and its equivalent negative form \ird{am luhninku} can be used,
with the main verb marked as a perfective converb.\index{converb}\footnote{Cf. the use of the perfective converb with the \textit{merci de} + infinitive construction in French. The use of \ird{am luhnink\'a} presupposes that the action being requested has already been done although in fact it hasn't, for which therefore the speaker is giving thanks. Thus, a simple request like \trsl{Please close the door} is expressed in Iridian as \trsl{May you be thanked for having closed the door.}}
...
The adhortative (\trsl{Let's}) is formed using \ird{lidovk\'a} with the imperfective converb form of the main verb. \ird{Lidovk\'a} can also be used by itself where the main verb may be implied from context, or as a reply to the request if the speaker wants to express agreement or assent.
...
\begin{table}[h!]
	...
	\begin{tabu} to 0.8 \textwidth	{Y[1.3]Y[1.3]YY}
		...
		\textit{Realis} 				&\ird{-i\v{c}} &v\'ine & jako\\
		Neg. \textit{Realis}		&\ird{-\v{c}nie}&ve&neko\\

		\textit{Irrealis} 			& \ird{-i\v{s}\v{c}e}& jenem & jenem\\
		Neg. \textit{Irrealis} 	& \ird{-i\v{s}\v{c}enie}& jet & n\'et\\
		\bottomrule
	\end{tabu}
\end{table}
	...
	\begin{tabu} to 0.9\textwidth {YYY}
		...
		\midrule
			...
			\addlinespace
				...
				\ird{pia\v{s}\v{c}an\'i} $\rightarrow$ \ird{pia\v{s}\v{c}\'anie}\\
			\addlinespace
				Continuous &
				\ird{-ev\'i} $\rightarrow$ \ird{\'ive} &
				\ird{pia\v{s}\v{c}ev\'i} $\rightarrow$ \ird{pia\v{s}\v{c}\'ive}\\
			\addlinespace
				Progressive &
				\ird{-\'ar} $\rightarrow$ \ird{\'ar\v{z}e} &
				\ird{pia\v{s}\v{c}\'ar} $\rightarrow$ \ird{pia\v{s}\v{c}\'ar\v{z}e}\\
			\addlinespace
				Contemplative &
				\ird{-\'ach} $\rightarrow$ \ird{\'a\v{z}e} &
				\ird{pia\v{s}\v{c}\'ach} $\rightarrow$ \ird{pia\v{s}\v{c}\'a\v{z}e}\\
			\addlinespace
				Prospective &
				\ird{-uj\'am} $\rightarrow$ \ird{-ujime} &
				\ird{pia\v{s}\v{c}uj\'am} $\rightarrow$ \ird{pia\v{s}\v{c}ujime}\\
			\addlinespace
				Cessative &
				\ird{-\'ov\'it} $\rightarrow$ \ird{-\'ov\'i\v{c}e} &
				\ird{pia\v{s}\v{c}\'ov\'it} $\rightarrow$ \ird{pia\v{s}\v{c}\'ov\'i\v{c}e}\\
			\addlinespace
...
\begin{table}[h!]
    ...
    \begin{tabu}to 0.6\textwidth{YYY}
			\toprule
				 ...
				 \midrule
         Debitive & \ird{-aln-} & \ird{-i\v{s}k-}\\
         Desiderative & \ird{-\'an-}&\ird{-u\v{s}h-}\\
         Potential &\ird{-\'et-} & \ird{-\'evn-}\\
			\bottomrule
    \end{tabu}
\end{table}
...
\pex
\a \irdp{pia\v{s}t\'a}{to eat}
\a \irdp{pia\v{s}taln\'a}{to need to eat}
\a \irdp{pia\v{s}ti\v{s}k\'a}{to not need to eat}
\a \irdp{pia\v{s}t\'an\'a}{to want to eat}
\a \irdp{pia\v{s}tu\v{s}h\'a}{to not want to eat}
\a \irdp{pia\v{s}t\'et\'a}{to be able to eat}
\a \irdp{pia\v{s}t\'evn\'a}{to not be able to eat}
\xe
...
\glb Tom\'a\v{s} Russian-\mk{inst} speak-\mk{av-pot-ctpv}//
\glft \trsl{Tom\'a\v{s} will be able to speak Russian}//
\endgl
\xe
