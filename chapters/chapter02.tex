...
\caption{Causative forms of the verb \irdp{shrad\'a}{to die.}}
    ...
    \begin{tabu}to \textwidth{Y[0.5]Y[0.6]YY}
         \toprule\addlinespace
		 		...
		 		Agentive				& \ird{neshr\'a\v{z}\'a}			& to die & to kill\\ \addlinespace
...
Perhaps due to this suppletive nature lexical causatives imply a more direct causation, or a tighter link between cause and event (\cite{velupillai2012}), than analytical or morphological causatives. Consider for example the three sentences in English below:
...
Iridian does not employ lexical causatives as in English; instead causatives are formed morphologically by adding the prefix \ird{ne-} (glossed as \mk{caus}) to the verb stem. Although the prefix \ird{ne-} is required to form the causative morphologically, some verbs, particularly stative verbs like \irdp{shrad\'a}{to die, to be dead} in table \ref{tbl:causative} may already contain the notion of causation in some of its regular conjugated forms. This is because by default stative verbs\index{stative verb} are intransitive (i.e., the only argument required is the actor/agent\index{agent}) while some verbal voices\index{voice} like the patientive\index{patientive voice} and benefactive\index{benefactive voice} inherently require two or three arguments\index{argument of a verb} respectively.

%% TODO add section reference

Of course Iridian's definition of which verbs are stative and which ones are dynamic\index{dynamic verb} does not neatly align with the definition those classes have in English (cf. \S~XX). For instance the verbs \emph{to stand} and \emph{to eat} are both dynamic verbs in English, while in Iridian \irdp{zdav\'a}{to stand, to be standing} is stative and only \irdp{pia\v{s}t\'a}{to eat} is dynamic. This is why as we see in example (\getref{stat-dyn}) below, some forms of the verb \ird{zdav\'a} already contain the notion of causation in some of its regular conjugated forms.

\pex
\a  \irdp{zdav\'a}{to stand, to be standing}\deftagex{stat-dyn}\\
