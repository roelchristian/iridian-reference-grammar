...
\par The citation form (or dictionary form) of a verb is the uninflected infinitive\index{infinitive}, a fossilized form rarely used outside of a very few periphrastic construction. The infinitive ends with the vowel \ird{-á}, and removing this ending will produce the verb stem\index{verb stem}. The final consonant  of the stem is called the thematic consonant and determines the conjugation paradigm the verb follows.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item stojá + -aš- $\rightarrow$ st\'ož-, \trsl{to go}
	\end{itemize}
...
\pex
\begingl
\gla Bych na gnaža Marek vdinek.//
...
\par The vowel in the suffix is unstable and the ending would normally collapse to \textbf{-k} when followed by another vowel. Consider the above two sentences followed by the second person singular clitic pronoun \textbf{-aš/eš}.

\pex
\begingl
\gla Bych na gnazsa Marek vindekeš.//
...
\glb friend-\mk{1s} smoke-\mk{av-cont-nz}//
...
Of course Iridian's definition of which verbs are stative and which ones are dynamic\index{dynamic verb} does not neatly align with the definition those classes have in English (v. \S\,\ref{sec:statives}). For instance the verbs \emph{to stand} and \emph{to eat} are both dynamic verbs in English, while in Iridian \irdp{zdavá}{to stand, to be standing} is stative and only \irdp{piaštá}{to eat} is dynamic. This is why as we see in example (\getfullref{statdyn.1}) below, some forms of the verb \ird{zdavá} already contain the notion of causation in some of its regular conjugated forms.

\pex
\a  \irdp{zdavá}{to be standing}\deftagex{stat-dyn}\\
		\irdp{zdavžá}{to stand}\deftagex{stat-dyn}\\
...
\glb vase Janek-\mk{gen} \mk{caus-}break-\mk{pv-pf}//
\glft \trsl{(I) made John break the vase.}//
\endgl
\a
\begingl
\gla Váz Janka nenopriznek.//
\glb vase Janek-\mk{pat} \mk{caus-}break-\mk{pv-pf}//
\glft \trsl{(I) made John break the vase.}//
\endgl
\xe
...
\pex
\begingl
\gla Karlu sodalšalím še Marek ščenžek.//
...
\glb \mk{dem} \mk{loc} church-\mk{pat} parents \mk{rec}-wed-\mk{av-pf}//
...
\glb \mk{pl=} senator \mk{rec}-shout-\mk{av-prog} \mk{rz} \mk{loc} television-\mk{pat} see-\mk{av-pf}//
...
\glb Mišek \mk{com} Martin-\mk{inst} \mk{rec}-know-\mk{av-prog}//
...
\glb \mk{2s.str} Mišek-\mk{inst} \mk{rec}-know-\mk{av-ret} \mk{q}//
...
\glb bread-\mk{gen} buy-\mk{pv-subj.ipf} {so:that} bakery go-\mk{lv-pf}//
...
\glb \mk{ref} see-\mk{pv-subj.ipf} {lest} leave-\mk{av-pf}//
\glft \trsl{(I) left so as not to be seen.}//
\endgl
\a
\begingl
\gla Vdinílá az varšek.//
\glb see-\mk{pv-subj.ipf} {lest} leave-\mk{av-pf}//
...
\begin{table}[h!]
	\footnotesize\sffamily
	\caption{Conjugation paradigm in the conditional mood for regular \\verbs, the copula and the existential particle \ird{ješ}.}
...
\glb water 100 Celcius-\mk{inst} \mk{caus}-heat-\mk{ben-cond.rl} \mk{ref-}boil-\mk{av-cont}//
...
\glb too:much drink-\mk{av-cond.rl} \mk{ref}-get:drunk-\mk{av-cont}//
...
\pex
\begingl
\gla Mém na prezna víne, dekání byr\'ova st\'ožka.//
...
\begin{table}
	...
	\begin{tabu} to 0.9\textwidth {YYY}
		...
		\midrule
			...
			\addlinespace
				Cessative &
				\ird{-\'ovít} $\rightarrow$ \ird{-\'ovíče} &
				\ird{piašč\'ovít} $\rightarrow$ \ird{piašč\'ovíče}\\
			\addlinespace
...
As in most languages, modal constructions in Iridian exhibit significant polysemy\index{polysemy} (i.e. a single construction can have one or more interpretation depending on the context). For example consider the following sentence:

\pex
\begingl
\gla Tomáš rušku zahviržétách.//
\glb Tomáš Russian-\mk{inst} speak-\mk{av-pot-ctpv}//
...
\pex
\begingl
\gla To švirek moc gruševí še kartounévnílá.//
...
The suffix \ird{-ál}, used to mark the continuous aspect, may be infixed to the gerund to indicate that the action is repetitive.
...
Converbs (glossed \mk{cv}) are a non-finite verb form often used for adverbial constructions. There are two converb forms in Iridian: the imperfective \ird{iec} (glossed \mk{cv.ipf}) and the perfective \ird{-u} (glossed \mk{cv.pf}).

\pex
\begingl
\gla Tereza kravniec ce nóve palžek. //
...
\pex
\begingl
\gla Ce nóve palzu Tereza neikravnašek.//
...
The {\sc supine}\index{supine} is a non-finite verb form formed used to indicate necessity or purpose. Both usage has a nominal and a non-nominal form, giving the supine a total of four forms, as shown below:
...
These four forms are invariable. The endings attach to the verb after the root has been conjugated for voice. The use of the non-nominal forms, in addition, does not require the use of the connecting particle \ird{ko}.

\pex
\a\begingl
\gla >>Ána Karenina<< za gna\v{z}a oštinášce t\'om.//
\glb Anna Karenina for school-\mk{pat} read-\mk{sup} book//
\glft `I have to read \textit{Anna Karenina} for school.'//
\endgl

\a\begingl
\gla T\'om behlenik to oštnice.//
\glb book buy-\mk{pv-pf} \mk{rz} read-\mk{sup}//
\glft `I bought the book to read.'//
\endgl
\xe

The supine\index{supine} is quite heavily employed in Iridian. In colloquial speech, the supine of purpose is often used to express future or probable events as a substitute to the contemplative aspect.

\pex
\begingl
\gla D\'a o\v{s}\v{c}it.//
\glb \mk{1s} read-\mk{sup}//
\glft `I will be reading.' \emph{Lit.,} `I am someone whose purpose is reading.'//
\endgl
\xe

Another common construction involves the supine of necessity with the words \irdp{shlac}{now} \irdp{m\'al}{time} (or less frequently \irdp{\'or}{hour}). This construction is somehow similar to English \trsl{It's time we left} or \trsl{It's time for us to go.} When used this way, the supine is conjugated in the locative voice.

\pex
\begingl
\gla Shlac himat\'i palzoun\'a\v{s}ce m\'al.//
\glb now homeland-\mk{gen} leave-\mk{av-sup} time//
\glft `It's time (we) left our homeland.'//
\endgl
\xe
\pex
\begingl
\gla Sa tet. Shlac zdaloun\'a\v{s}ce m\'al.//
\glb already noon time have:breakfast-\mk{av-sup}//
...
\begin{table}[h!]
	...
	\begin{tabu} to 0.7\textwidth{YY[0.8]Y}
		...
		Attributive & \ird{-\'i} & \ird{v\v{s}ihn\'i}\\
		Nominative & \ird{-ou}	& \ird{v\v{s}ihnou}\\
		\bottomrule
	\end{tabu}
\end{table}
...
\ex
\begingl
\gla V\v{s}ihninek ko tieho snov upro\v{z}ilz\'ach.//
\glb be:angry-\mk{pv-pf} \mk{att} god soon \mk{ref-}avenge-\mk{av-ctpv} //
...
The copulative form may also be nominalized with \ird{-ou}. However, as with the attributive form, if the copulative stative verb is conjugated in the continuous aspect and is unmarked for voice, the nominal form is used instead of the nominalized copulative form.
