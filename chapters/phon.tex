...
\begin{table}[h!]
	...
	\begin{tabu} to \textwidth{Y[2]MMMM}
		...
		\midrule
		Plosive & p~b		& t~d	 		& (c~\textbardotlessj)	& \textipa{k~g} \\
		Nasal	& \textipa{m~(M)}			& n				& (\nn)	& 	\textipa{(N)}	\\
		Liquid	&				& r~(\textipa{\rrr})~l				&	(\textipa{L})	&	\\
		Sib. Fric.& 			&s~z	  & \textipa{C~\textctz} &\\
		Non-Sib. Fricative& 		v	&	&   (ç) &\textipa{x~(G)}\\
		Sib. Affricate &				& \textipa{\jn{ts}~(\jn{dz})}	& \textipa{\jn{tC}~(\jn{d\textctz})}  &  \\
		Non-Sib. Aff. &				& \textipa{(\jn{\|[tT}~\jn{\|[dD})}	& \textipa{(\jn{cç}~\jn{\jjg J})}  & \textipa{(\jn{kx}~\jn{gG})} \\
