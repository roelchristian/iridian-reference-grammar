...
\subsection{Nasals}
Iridian has three nasal consonants \bt{m~n~\nn}. \bt{n} cannot appear before bilabials and similarly \bt{m} cannot appear before velars. Both \bt{m} and \bt{n} are realized as \nt{M} before either \bt{V} or \bt{f}. Before velars \bt{n} is consistently realized as \nt{N}, although \nt{n} is also possible in emphatic pronunciation or in word boundaries.

The velar \nt{N} is not native in Iridian but can sometimes be observed, especially in loanwords, where it can be realized as nasalization of the preceding vowel when in the syllable coda or as \nt{N} intervocalically, although \nt{Ng} or \nt{Nk} is also common. Thus, for example, \irdp{aktung}{signage} (from Ger. \emph{Achtung}) can be realized as \nt{"Paxt\~u} or \nt{"PaxtuN} or \nt{"PaxtuNk}; or \irdp{anglevn\'i}{English} as \nt{"P\~5lEV\nn{}i:} or \nt{"PaNlEV\nn{}i:} or \nt{"P5NglEV\nn{}i:}.


\subsection{Liquids}

Iridian has three liquids: the rhotic \bt{r} and the lateral \bt{l} and \bt{L}.
