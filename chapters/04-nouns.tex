...
\begin{itemize}
	...
	\item the \term{zero-grade stem}, where an unstable \ird{e} is deleted before a vowel-initial ending;
	\item the \term{raised stem}, where zero-grade formation further triggers \ird{e/ě/é $\rightarrow$ i/í} or \ird{o $\rightarrow$ u};
	\item the \term{soft stem}, where a front-vocalic ending palatalizes the final consonant.
\end{itemize}

Not every noun shows all four grades, but every morphophonemically complex noun can be described in these terms. Dictionaries should therefore list nouns by three principal parts: \term{Direct}, \term{Transitive}, and \term{Indirect}.
