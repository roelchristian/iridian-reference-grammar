...
should therefore not be analyzed as members of the Class~1--3 particle series,
...
Pol\textsuperscript{0} in §\,\ref{sec:by-pol-head}; the full syntax of polar
...
Class~2 inventory in the present analysis and treated in
§\,\ref{sec:by-pol-head} immediately below; the formal distinction is
explained there.
...
with \ird{v} is the more lexicalized of the two. Since \ird{v} presents the
...
The combination with \ird{s} is less lexicalized and remains more transparently
...
relevant surprise may come from new information, abrupt realization,
...
\subsection{\ird{By}: the canonical exponent of Pol\textsuperscript{0}}
\label{sec:by-pol-head}

The particle \ird{by} stands apart from the Class~2 inventory
in a structurally important respect. While the Class~2 particles occupy
the AspP-level comment domain, \ird{by} is the canonical overt exponent of
Pol\textsuperscript{0}, the head of \textsc{PolP} in the clause spine
(§\,\ref{sec:th-clause-structure}). PolP is a dedicated clause-typing
projection situated above FocP and below TopP; it is therefore structurally
higher than the comment domain. Retaining \ird{by} in the Class~2 list would
contradict the hierarchy in Table~\ref{tab:th-clause-spine}, where PolP is a
distinct projection above the comment-domain tier. The older three-zone
descriptive presentation, which placed \ird{by} in the ``middle'' (Class~2)
slot, captured a valid surface generalization, but its formal interpretation
must now be set aside in favour of the PolP analysis.

The analysis of \ird{by} as Pol\textsuperscript{0} entails the following
structural claims. First, \ird{by} is the overt exponent of Pol\textsuperscript{0}
in matrix interrogatives generally: it types the whole clause as interrogative
and scopes over the full IP, including any comment-domain material. Second,
Pol\textsuperscript{0} may be phonologically null. In polar questions, overt
\ird{by} is the ordinary and unmarked explicit strategy; in content questions,
both overt \ird{by} and null Pol\textsuperscript{0} are standardly available
...
rather than a grammatical asymmetry. Third, Pol\textsuperscript{0} is not a
focus-licensing head; Spec,FocP remains a distinct position hosting a fronted
...
focus position is independent of whether Pol\textsuperscript{0} is overt or
null.
...
clause-level interrogative force. Because PolP is above FocP, which is above
AspP and PhasP, the higher particles (EvidP, SpeechActP) naturally scope
...
source. These scope facts are exactly what the PolP position predicts and
