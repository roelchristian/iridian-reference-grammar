...
but a shared positional behaviour and a fixed hierarchy of scope.
...
\subsection{\ird{By}: the canonical exponent of \synhead{Pol\textsuperscript{0}}}
...
the \synhead{AspP}-level comment domain, \ird{by} is the canonical overt exponent of
\synhead{Pol\textsuperscript{0}}, the head of \synhead{PolP} in the clause spine
(§\,\ref{sec:th-clause-structure}). \synhead{PolP} is a dedicated clause-typing
projection situated above \synhead{FocP} and below \synhead{TopP}; it is therefore structurally
higher than the comment domain. Retaining \ird{by} in the Class~2 list would
contradict the hierarchy in Table~\ref{tab:th-clause-spine}, where \synhead{PolP} is a
...
slot, captured a valid surface generalisation, but its formal interpretation
must now be set aside in favour of the \synhead{PolP} analysis.
