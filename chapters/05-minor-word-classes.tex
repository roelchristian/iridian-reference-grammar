...
\begin{itemize}[nosep]
	...
	\item Class 3: \ird{izdy}, \ird{oče}, \ird{nadě} and \ird{děne}
\end{itemize}
...
\paragraph{izdy} \ird{Izdy} is used to express that the speaker has deduced the
...
The use of \ird{izdy} requires two conditions: (1) the utterance must be
directly based on some truth already known to the speaker and (2) the speaker
must be unsure whether or not the utterance itself is true. This makes the usage
of \ird{izdy} inferential in nature instead of merely expressing some doubt or
...
agent \ird{Jankám} but before the verb \ird{kupenik}. Viz.,
...
\begingl
	...
	\glft \trsl{This looks like what Janek has bought (contrary to what we might have thought before, though I'm still not sure).}//
\endgl
\xe
