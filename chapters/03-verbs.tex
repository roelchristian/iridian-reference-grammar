...
\noindent The complete distribution, including mood-internal negation, is
summarized in Table~\ref{tab:neg-exponents} of Chapter~\ref{ch:simple}.
The three environments above --- verbal, equative, and existential --- are
a conditioned allomorphy of a single
...
incorporation at PF --- makes the following empirical predictions
that can be tested directly: (a) \ird{Zá-} does not form an
independent prosodic word; it is a bound clitic on the verbal
complex and cannot receive independent stress or be separated by
an intervening focus particle. (b) Nothing may intervene between
\ird{zá-} and the stem; if focus or evidential particles can split
the two, the cliticization analysis would need revision. (c) In
ellipsis or fragment contexts, \ird{zá-} cannot appear stranded
without the full verbal complex (unlike a free-standing negative
head, which should be able to survive ellipsis of its complement).

Two additional predictions bear on the three-exponent analysis
developed in §\,\ref{sec:vm-neg-exponents}: (d) \ird{Staly} is
fully invariant, bearing no aspect, person, mood, or iterative
marking. If future data reveal paradigmatic variation, the
Neg\textsuperscript{0} analysis would need revision in favour of
a defective verbal root. (e) The conditioning environments for
the three exponents (\ird{zá-}, \ird{staly}, \ird{zad-}) are
strictly complementary, delineated by the presence or absence of
a finite verbal host (see §\,\ref{sec:vm-tension-stative} for the
equative/stative boundary, which is now explicitly resolved). Any
attested overlap context --- a clause where the predicate is
ambiguous between equative and stative --- would require the
complementarity claim to be refined.
