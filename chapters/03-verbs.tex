...
MoodP [+\textsc{neg}] is directive-scoped: it negates the force or
modality encoded by the MoodP head (a command, a suggestion, a
hypothetical situation), not a proposition. A prohibitive such as
\ird{piaštéb-é} \trsl{Don't feed anyone!} does not have
truth conditions in the ordinary sense; its negation qualifies the
directive illocution, not a predicate. Likewise, the negative
irrealis \ird{piaštir} denies the hypothetical or counterfactual
applicability of the event, rather than asserting that a proposition
is false. The two types of [+\textsc{neg}] are formally parallel in
their blocking of NegP projection; they are semantically distinct in
scope and illocutionary function.
