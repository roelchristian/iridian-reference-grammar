...
The five Asp.Align categories fall into two coherent higher-order domains. The
\term{perfective domain} comprises the \term{perfective} and the
...
\term{continuous}, the \term{progressive}, and the \term{contemplative}: none of
these presents the eventuality as a bounded whole viewed from outside; instead,
each either locates the reference point within the event's internal course or
presents the event as projected relative to reference time, not yet realised but
anticipated, intended, or impending. All three yield \term{nominative
alignment}, in which the agent is the absolutive pivot. These two domains differ
not merely in which aspects they contain but in the fundamental orientation they
impose on the clause: the perfective domain foregrounds culmination and the
affected patient, while the imperfective domain foregrounds the ongoing or
prospective engagement of the agent.
...
realise a transitive clause. An overt Voice head is additionally required. The
...
cannot be realised as a core argument, and the bare perfective or retrospective
...
\noindent where the reduced Asp.Align is silent ($\varnothing$) in the
...
and the palatalizing irrealis suffix \ird{-ěv}, stem palatalisation
...
Syntactically, Mood remains the higher head (\synhead{MoodP} $>$ \synhead{AspP}), consistent with
...
surfaces on a finite verb unless Asp.Align has been realised. The interaction is
...
\noindent Where a patient is to be expressed in any form, the ergative construction is required. Instrument and adjunct arguments, which bear no patient relation to the predicate, remain fully admissible in the indirect case, as in the spoon example above. The absence of an expressible patient is not merely a discourse choice to background a participant that is otherwise retained in syntax: under the antipassive, the patient is removed from the clause's core argument structure and therefore cannot surface in any case form. The construction does not merely withhold the patient from prominence; it eliminates the patient from the overt argument structure of the clause. For an inherently transitive verb the patient is typically still understood as part of the conceptual event, but what is asserted is the agent's activity; the patient has no realisation as a syntactic argument.
...
may be merged only in a clause whose input to \synhead{VoiceP} contains a
patient that is eligible for absolutive realisation. It then removes
...
confirms that \synhead{VoiceP} immediately dominates \synhead{VP}:
...
head represents the input condition: the head selects for a \synhead{VP} whose
...
the same \synhead{VoiceP} projection: only one Voice head may be merged at this
tier.

In ERG-aspect transitives, the patient requires overt Voice licensing
to be realised as a core argument (§\,\ref{sec:vm-n}). A verb bearing
neither \ird{-aš-} nor \ird{-éb-} nor \ird{-n-} in an ergative
aspect therefore fails for a different reason: the patient lacks the
overt patient-licensing head required for core realisation. That
...
\noindent In the imperative and hortative the same promotion yields benefactive readings by default. Directive clauses require the beneficiary to be overtly realised; the applicative is ungrammatical without one (see §\,\ref{sec:mood-directive-valency}):
...
(2008), this can be analysed as a
...
applicative becomes the absolutive argument of the resulting \synhead{vP},
effectively taking the slot that would otherwise be occupied by the
patient. The interaction with discourse is mediated at the \synhead{AspP}
...
\noindent Synchronically, \ird{-na-} is distinct from the Voice head \ird{-n-}. The former is a valency-changing derivation occupying the causative slot of the verbal template (\synhead{CausP}); the latter is the Voice head (\synhead{VoiceP}) that licenses the internal argument and specifies patient absolutive, inserted immediately before the aspect suffix. Earlier notes used \ird{-ne-} for the causative, but the present grammar regularises \ird{-na-} in order to keep the causative head formally distinct from \ird{-n-}.
...
before Asp.Align) is predicted: \synhead{CausP} dominates \synhead{VoiceP}/\synhead{ApplP}, and
the Mirror Principle assigns them to the innermost positions relative
to \synhead{AspP}. The interaction with aspect is semantically significant
(§\,\ref{sec:aspect-caus}): perfective causation presents direct
causer control (\synhead{CausP} features trigger tight Agree with the event
...
without an overt Voice head in \synhead{VoiceP}, a transitive predicate cannot
realise its patient as a properly licensed participant in the clause's
...
realisation upon the clause. The full analysis and examples are in
§\,\ref{sec:vm-passive}.
...
nominative-domain clause is reorganised around the patient, allowing
...
and the contemplative \ird{-ách}; the patient-absolutive
specification of \ird{-n-} is realised in each case.
...
Within the clause spine (Table~\ref{tab:th-clause-spine}), \synhead{NegP}
occupies projection 4, above \synhead{AspP} but below \synhead{IP}. If the Mirror
Principle held perfectly, \ird{zá-} should appear \emph{after} the
Asp.Align suffix, since \synhead{NegP} dominates \synhead{AspP} and therefore corresponds
...
\synhead{NegP} is a syntactic head above \synhead{AspP}, but \ird{zá-} is specified as a
...
\ird{zá-} as the sole reflex of \synhead{NegP} is incomplete: Neg\textsuperscript{0}
has three PF exponents conditioned by predicate type. With a finite
verbal predicate, Neg\textsuperscript{0} is realised as the procliticized
...
DM readjustment: the existential root is realised as the suppletive
...
Insertion --- the standard behaviour of suppletive negative existentials
...
summarised in Table~\ref{tab:neg-exponents} of Chapter~\ref{ch:simple}.
...
\synhead{NegP} occupies the same structural position (above \synhead{AspP}, below \synhead{MoodP})
in all clause types; what varies is the PF exponent of
Neg\textsuperscript{0} and the semantic scope of negation, which is
determined by the semantics of the predication \synhead{NegP} takes in its scope.
...
evidence for the head-final status of \synhead{NegP}. In existential clauses,
...
canonical behaviour of Neg\textsuperscript{0}.
...
\ird{-ir}) parallels the \ird{zad-} case in its effect on \synhead{NegP} projection: when
\synhead{MoodP} carries [+\textsc{neg}] as a VI-internal feature, \synhead{NegP} is absent from the
derivation and \ird{zá-} accordingly does not surface. The full analysis of
\synhead{MoodP} [+\textsc{neg}] --- including the semantic distinction between
directive-scoped and propositional negation, and the restricted palatalizing
behaviour of \ird{-ir} --- is in §\,\ref{sec:vm-moodp}.
...
contrast can be stated precisely in terms of what \synhead{NegP} takes in its scope.
...
version (\ird{Marek doktor staly}) denies property-membership. \synhead{NegP} takes a
...
\term{identificational}: it asserts that the topic \synhead{DP} is the unique entity
satisfying the nominalised description. The semantic content of the predicate
(the participant nominal) is inherently exhaustive --- it picks out exactly one
referent. \synhead{NegP} over a specificational predication yields \term{identificational
negation}:
...
identificational \ird{staly}+nominal is not ambiguity in the \synhead{NegP} head but a
difference in what \synhead{NegP} scopes over. \ird{Zá-} scopes over a finite event
...
Chapter~\ref{ch:nouns}) are not part of the clausal \synhead{NegP} system described here.
Their interaction with the \synhead{NegP} projection --- in particular, whether they
project their own \synhead{NegP}, co-occur with \ird{zá-}, or block \synhead{NegP} projection --- is
...
negative value of the \synhead{MoodP} Vocabulary Item, rendering \synhead{NegP} absent from the
...
Vocabulary Item at \synhead{MoodP} rather than a suppletive form in the strict sense. Like
...
palatalisation and no associated vowel raising. Thus the same stem that yields
...
The mood suffixes occupy the projection immediately outside \synhead{AspP}. In the
indicative, \synhead{MoodP} is present but headed by a null morpheme. In the
non-indicative moods (imperative, hortative, irrealis), a lexical mood morpheme
is inserted at \synhead{MoodP} and simultaneously triggers a post-syntactic impoverishment
...
reduced Asp.Align (see §\,\ref{sec:vm-mood-asp}). \synhead{AspP} is therefore always
...
mood paradigm consists of competing Vocabulary Items inserted at the \synhead{MoodP} head:
...
When a \synhead{MoodP} head bearing [+\textsc{neg}] is selected, \synhead{NegP} is absent
from the derivation entirely: the clause is assembled with the negative \synhead{MoodP}
merging directly above \synhead{AspP}, and no Neg\textsuperscript{0} head enters the
...
of \synhead{NegP} not being projected.

An important asymmetry follows. The indicative is the only mood that projects
\synhead{NegP} and uses \ird{zá-}: it is the only cell in the \synhead{MoodP} paradigm whose
...
negative VI, rendering \synhead{NegP} superfluous in those environments.

The semantic character of \synhead{MoodP} [+\textsc{neg}] differs from that of \synhead{NegP}
[+\textsc{neg}]. \synhead{NegP} negation is propositional: it operates over a
truth-evaluable event description and inverts its truth conditions. \synhead{MoodP}
[+\textsc{neg}] is \term{illocutionary/modal-scoped}: it negates the force or
modality encoded by the \synhead{MoodP} head, not a proposition. Within this class, two
...
rather than prohibiting it. The two sub-types of \synhead{MoodP} [+\textsc{neg}] are
formally parallel in their blocking of \synhead{NegP} projection; they are semantically
...
\term{inner evidential} (\synhead{EvidP}$_\text{inner}$) is distinct from the clausal
...
precedes the complement linker \ird{-e} and follows \synhead{MoodP}, giving the order:
...
\noindent The corresponding syntactic projection is therefore above \synhead{MoodP} and
constitutes the outermost layer of the finite inflectional spine. The boundary
with the clausal \synhead{EvidP}$_\text{outer}$ (a left-peripheral projection) must be
kept clear: the inner evidential marks the \emph{source of knowledge} as it
bears on the proposition expressed by the verbal complex; the outer \synhead{EvidP}
...
The simultaneous/manner converb \ird{-ěc} and the irrealis mood \ird{-ěv} belong to the palatalizing domain, while the antipassive \ird{-aš-} is the clearest instance of true sibilant replacement. The conditional \ird{-ic} does not palatalise despite its initial front vowel; the historical reasons for this exception are discussed in Section~\ref{sec:cond-nopala} of Chapter~\ref{ch:complex}. Stronger irregularity is concentrated instead in a few high-frequency linker and quotative forms, where analogical levelling has obscured the older outputs.
...
converb suffix therefore projects a \synhead{vP}/\synhead{VoiceP} but no \synhead{AspP}. The
converb suffix itself can be analysed as the Vocabulary Item
...
Alternatively, converbs project no \synhead{AspP} at all: the converb suffix
is the head of a Voice or \synhead{vP} projection that merges directly with
the higher adverbial or complementizer position without passing
through \synhead{AspP}. The absence of the ergative case marker \ird{-n-}
inside converb clauses (§\,\ref{sec:converb-chains}) is predicted
by either account: if \synhead{AspP} is absent, the Asp head that would have
...
adjuncts to the matrix \synhead{IP} or \synhead{vP}, not as arguments in Spec or
Complement positions. This explains their preverbal (and pre-matrix)
position: as adjuncts to \synhead{IP}, they precede all material within \synhead{IP}
...
cases all internal arguments of the nominalised event appear in the indirect
case, since the absolutive/direct slot of the equative clause is occupied
by the external topic \synhead{DP} and no verbal case-assigning machinery operates
within the nominalised predicate.

The semantic effect of predicating a participant nominal is
\term{identificational}: the topic \synhead{DP} is being identified as the unique
...
The exact output depends on the segment immediately preceding the linker. Velars typically yield \orth{c/ce}; other endings show the regular soft reflexes of the final consonant or syllable in standard spelling, without introducing separate haček letters for soft \orth{t}, \orth{d}, or \orth{l}. Some older conjunctive formations are analogically levelled and remain morphologised, but the synchronic generalisation is clear: \ird{-e} selects a palatalised form of the preceding material. The formal syntactic analysis of the linker is developed in the following subsection.
...
behaviour of \ird{-e} with its morphosyntactic distribution and its
...
productive front-vocalic palatalisation system described in
§\,\ref{sec:front-palatalisation}. Its trigger class is the same as
...
palatalisation, because it was grammaticalised during a period when
...
generalisation that \ird{-e} is a palatalizing trigger in the
productive system.
...
\begin{itemize}
  \item \textit{Velars} (/k/, /g/): the velar yields the palatalised
    affricate series. The most common output is \orth{c} for /k/,
    as in \ird{piašček} $\rightarrow$ \ird{piaščice} (the final
    velar of the perfective suffix \ird{-ek/-ik} palatalises to
    \orth{c} before \ird{-e}).
  ...
  \item \textit{Labials} (/p/, /b/, /m/, /v/) and liquids (/r/):
    secondary palatalisation (a palatal off-glide) is added, giving
    the same soft-series output as before nominal \ird{-í}
    (§\,\ref{sec:palatalisation}). These outcomes are less
    phonologically salient than velar or coronal softening and are
    typically not marked distinctively in orthography.
\end{itemize}

\noindent The key generalisation is that \ird{-e} selects the soft
...
predictable from the palatalisation system already operative in
...
suffix raises to \ird{-i-} before the palatalised output, yielding
...
palatalisation is thus not specific to \ird{-e}: it is a general
...
Some high-frequency conjunctive forms are morphologised: they
preserve older palatalisations or otherwise deviate from the
...
\noindent The linker is therefore exterior to \synhead{EvidP}$_\text{inner}$, the
...
\begin{enumerate}
  \item \textit{The finite \ird{po}-complement}
    ...
    the embedded finite \synhead{TP} before the complement C head \ird{po} or
...
\synhead{TP}, the linker is not licensed. Second, converb forms --- the
...
\begin{description}[leftmargin=2em, labelwidth=!, labelindent=0em]
  \item[\upshape Unified \synhead{CP}-edge analysis.] Under this analysis,
    ...
    valued on the embedded \synhead{TP} when it enters the scope of a C
  ...
  \item[\upshape PF boundary-marker analysis.] Under this analysis,
    ...
    segmental mutation. The palatalisation caused by \ird{-e} is
    identical in type to the palatalisation triggered by inflectional
  ...
  \item[\upshape Two-homophone analysis.] Under this analysis,
    ...
    class. \ird{-e}$_1$ is the exponent of the \synhead{TP}-\synhead{CP} boundary
...
subordination' --- on the embedded \synhead{TP}'s outermost layer, and that
this feature is valued whenever a C head (overt or null) selects the
finite \synhead{TP}. The palatalizing nature of \ird{-e} is then the PF
realisation of this morphosyntactic feature, fully parallel to the
...
inflectional domain, which is exactly where a \synhead{TP}-edge or C-interface
...
\begin{enumerate}
  \item \textit{The null conjunctive C$^0$.} The postulated null C
    ...
    \synhead{TP} as its specifier and itself projects a \synhead{CP}, then the chained
    clause is a \synhead{CP} adjunct. This predicts that binding relations and
  ...
  \item \textit{Interaction with the evidential.} The indirect
    ...
    clarify whether the palatalisation is invariant (expected under
...
edge, but whose phonological behaviour is fully integrated with the
...
noun within the \synhead{DP}, and the \ird{-ě} form modifies the event as a manner
...
\medskip
\noindent The two clause types are superficially similar: both surface as [\synhead{DP}
...
\begin{description}[leftmargin=2em, labelwidth=!, labelindent=0em]
\item[\upshape A-movement to Spec,\synhead{TopP}.] The absolutive \synhead{DP} moves
  from its base position (complement or specifier of \synhead{VP}/\synhead{vP}) to
  Spec,\synhead{TopP} in the narrow syntax. This movement is driven by a
  [\textsc{top}] feature on the Top head, which probes for a
  matching \synhead{DP}. Case is assigned in Spec,\synhead{TopP} (absolutive as
  a structural case of the topic position). This is formally
  comparable to subject-raising analyses in familiar nominative
  languages, but in Iridian the moved \synhead{DP} is the absolutive pivot
  rather than a language-wide subject.
...
The formal analysis of the conjunctive linker \ird{-e} --- including its phonological characterization, morphosyntactic distribution, the competing unified \synhead{CP}-edge, PF boundary-marker, and two-homophone analyses, and the outstanding empirical questions --- is developed in full in the conjunctive linker section above (§\,\ref{sec:conjunctive-linker}).

\subsection{The NEG prefix vs.\ \synhead{NegP} position: see §\,\ref{sec:vm-neg}}
...
the highest projection (\synhead{SpeechActP}) to the lowest (\synhead{VP}/Root), with
...
applied at the \synhead{VP} level. No movement or extraposition is required:
the complement is base-generated to the left of the verb and
surfaces there in the linear string. Higher projections (\synhead{AspP},
\synhead{NegP}, \synhead{MoodP}, and the inner evidential) are realised as suffixes on the verbal
