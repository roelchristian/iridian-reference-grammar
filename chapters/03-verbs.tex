...
\ird{-éma}, hortative \ird{-ká/-ku}, irrealis \ird{-ěv}), the
...
negative mood forms (prohibitive \ird{-éma}, negative hortative
...
\ird{-éma}, hortative \ird{-ká/-ku}, irrealis \ird{-ěv}) are
...
includes both underived stative verbs such as \ird{shodn-} \trsl{wear} in its
stative sense and stative predicates derived by the verbalising suffix
\ird{-n-} from a closed set of lexical property roots. Stative verbs are true
verbs and accordingly require no copula to function as predicates.
...
The same aspectual pattern holds for underived stative verbs:
...
\gla Vytam shoda.//
\glb shirt.\Dir{} wear-\Cont{}//
\glft \trsl{(I) am wearing a shirt.} \lingcontext{continuous: ongoing state}//
\endgl
\a
\begingl
\gla Vytam shodime.//
\glb shirt.\Dir{} wear-\Prog{}//
\glft \trsl{(I) am putting on a shirt.} \lingcontext{progressive: coming to be in the state}//
\endgl
\xe
...
\subsection{Attributive forms}\label{sec:stative-attr}

\subsubsection{The direct attributive in \ird{-á}}
...
\subsubsection{Derived manner adverbs in \ird{-ě}}
\label{sec:stative-adv}
...
This formation is derivational rather than inflectional: it yields a lexical
adverbial adjunct, not a further verbal form, and it is distinct both from the
adnominal attributive and from the converb system discussed in
Chapter~\ref{ch:complex}. Schematically, the pattern is as follows:
...
\subsubsection{Superlative: \ird{-ěsk-}}

A superlative or intensifying degree is formed by inserting \ird{-ěsk-}
between the root and the \ird{-á} ending:

\pex
\a \ird{hradněská} \trsl{very good, extremely good}
\a \ird{mrocněská} \trsl{very beautiful, extremely beautiful}
\xe

\noindent The \ird{-ěsk-} suffix is confined to the \ird{-á} attributive and
does not attach to the finite stative verb stem.
...
\subsection{Stable and temporary states}\label{sec:stative-stability}
...
\subsection{The aspect system and the structure of stative verbs}
\label{sec:vm-tension-stative}

Stative verbs (§\,\ref{sec:stative}) present a specific challenge
for the Asp.Align analysis. The continuous aspect of a stative
verb expresses a stable state (property predication), but the
progressive of the same verb expresses dynamic entry into the
state (inchoative). This means the Asp heads carry aspectual
distinctions (stative vs.\ dynamic) even for predicates whose
lexical root does not specify transitivity. The alignment issue is
moot for intransitive predicates (both agent and absolutive
coincide), but the broader question is whether the full five-way
Asp paradigm applies uniformly to stative roots or whether some
Asp heads are blocked or semantically reinterpreted when the root
is stative. This is a gap in the current analysis.
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\textwidth}{l l l L}
    ...
    & \textsc{MoodP} & \ird{-ím, -éma, -ká, -ku, -ěv}, $\emptyset$
