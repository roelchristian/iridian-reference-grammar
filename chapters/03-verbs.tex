...
\item ERG-aspect domain (perfective, retrospective). The patient receives the
  direct (absolutive) case and the agent the transitive (ergative) case. The
  ergative is the marked, dependent case: it is assigned to the agent only when
  a patient has already received absolutive. The absolutive is therefore the
  \emph{patient}.
...
The defaults above may be overridden by an overt Voice head. When a transitive
stem bears a Voice head carrying its own absolutive specification, that
specification supersedes the Asp.Align default. The morpheme \ird{-n-} is such a
head: it licenses the internal argument and forces patient-absolutive, yielding
ergative transitives in the ERG domain and passive constructions in the NOM
domain (§\,\ref{sec:vm-n}, §\,\ref{sec:vm-passive}). The antipassive \ird{-aš-}
...
the aspect domain. Asp.Align determines the default; Voice, when present,
narrows or redirects that default within the same domain.
